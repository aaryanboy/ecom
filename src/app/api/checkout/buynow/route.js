...
export async function POST(req) {
  try {
    ...
    if (!userId || !productId || typeof quantity !== 'number' || quantity < 1) {
      return NextResponse.json({ success: false, message: 'userId, productId and valid quantity required' }, { status: 400 });
    }
    ...
    // Encode product details into transaction UUID suffix for verification
    // Format: <ts>-<uuid>-<userEmail>|pid:<productId>|qty:<quantity>
    const transactionUuid = `${Date.now()}-${uuidv4()}-${userId}|pid:${productId}|qty:${quantity}`;
    ...
    console.error('Error initiating buy-now checkout:', error);
    return NextResponse.json({ success: false, message: 'Failed to initiate buy-now checkout' }, { status: 500 });
  }
}
